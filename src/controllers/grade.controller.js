...
class GradeController {
  ...
  // Criar atividade (prova ou trabalho) e adicionar ao Grade
  async criarAtividade(req, res) {
    try {
      ...
      // Verificar permissão
      if (role === "professor") {
        if (grade.professorId.toString() !== teacher._id.toString()) {
          return res
            .status(403)
  ...
  async list(req, res) {
    try {
      ...
      const grades = await Grade.find(query)
        ...
        .populate("atividades", "nome tipo valor data status")
  ...
  async update(req, res) {
    try {
      ...
      await GradeController.recalcularMedia(gradeAtualizado);
  ...
  static async recalcularMedia(grade) {
    try {
